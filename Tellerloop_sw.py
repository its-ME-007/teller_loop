...
# ===== MQTT Failure recovery Function =====

def flush():    
    """Execute Flush capsule procedure"""
    global system_busy, current_task_id
    
    system_busy = True
    log("Starting flush procedure")
    
    # Notify system that we're beginning the flush procedure
    ack_data = { 
        "station": STATION_NAME,
        "status": "flush",
        "task_id": current_task_id,
    ...
    sensors = read_sensors()
    log(f"Sensor status: {sensors}")
    
    # Handle different sensor positions
    if sensors['P1']:
        log("Flush: Capsule at P1, moving to drop (S1)")
        # Wait for capsule at P1
        log("Waiting for capsule at P1...")
        while GPIO.input(P1) == GPIO.LOW:  # Wait until sensor is triggered (LOW)
            time.sleep(0.1)
        ...
        log("Flush: Capsule at P2, moving to P3 (S2)")
        move_motor(GPIO.HIGH, S2)
        time.sleep(0.5)
        log("Waiting for capsule at P3...")
        while GPIO.input(P3) == GPIO.LOW:  # Wait until sensor is triggered (LOW)
        ...
        log("Flush: Capsule at P3, activating blower to push forward")
        log("Waiting for capsule at P3...")
        while GPIO.input(P3) == GPIO.LOW:  # Wait until sensor is triggered (LOW)
            time.sleep(0.1)
        log("Capsule detected at P3")
        ...
        log("Flush: Capsule at P4, moving to reset position (S2)")
        move_motor(GPIO.HIGH, S4)
        time.sleep(2)
        log("Capsule received")
        ...
        log("Flush: No capsule detected or unknown position")
    
    # Notify system that flush is complete
    completion_data = {
        "type": "flush",
        "station": STATION_NAME,
        "task_id": current_task_id,
...
def handle_mtn_message(payload):
    ...
    try:
        ...
        if action == "self_test":
            ...
            current_dispatch_mode = "flush"
            motor_thread = threading.Thread(target=flush, daemon=True)
            motor_thread.start()
