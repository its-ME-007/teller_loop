...
# Modified clear_history endpoint with timestamp
...
def clear_history_7_days():
    ...
    try:
            ...
            if comp_type in ['passthrough-station', 'bottom-loading-station']:
                if comp_id:
                    ...
                    if cursor.fetchone():
                        ...
                        logger.info(f"Deleted {deleted_count} records older than 7 days from {table_name}")
                    else:
        ...
        db.commit()
        logger.info(f"Cleared data older than 7 days from {tables_processed} tables")
        
        return jsonify({
            'status': 'success',
            'message': f'History older than 7 days cleared successfully.',
...
def clear_history_30_days():
    ...
    try:
            ...
            if comp_type in ['passthrough-station', 'bottom-loading-station']:
                if comp_id:
                    ...
                    if cursor.fetchone():
                        ...
                        logger.info(f"Deleted {deleted_count} records older than 30 days from {table_name}")
                    else:
                        logger.warning(f"Table {table_name} does not exist")
                else:
                    logger.warning("Skipping component due to missing id")
        ...
        db.commit()
        logger.info(f"Cleared data older than 30 days from {tables_processed} tables")
        
        return jsonify({
            'status': 'success',
            'message': f'History older than 30 days cleared successfully.',
            'tables_processed': tables_processed,
            'records_deleted': total_records_deleted,
            'cutoff_date': cutoff_date.isoformat()
        }), 200
