...
# MQTT Configuration
mqtt_broker_ip = "127.0.0.1"  # ✅ for local Mosquitto broker
...
def handle_resume_process(data):
    ...
    if process_type == "dispatch":
        print(f"🔁 Resuming dispatch for station {station_id} with data: {task_data}")
        # Call the existing dispatch logic
        socketio.emit('dispatch', task_data, room=str(station_id))  # Optional
        handle_dispatch(task_data)  # Call your existing function directly
    else:
        print(f"⚠️ Unknown process type in resume: {process_type}")
