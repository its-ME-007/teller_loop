...
function toggleScreensOnClick(buttonid) {
  ...
  window.setDispatchCircles = function (requesterStation, acceptorStation) {
    ...
    if (typeof selectedDestination !== 'undefined') {
     ...
     lastKnownState = {
        station_id: receiverNumber, // receiving station ID
        process_type: "dispatch",
        task_data: {
          from: acceptorStation,
          to: requesterStation,
          priority: "low",  // update if needed
          timestamp: Date.now()
        }
      };
...
let lastKnownState = null; // You must set this when a task starts
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  socket.on('disconnect', () => {
    ...
    const statusBox = document.getElementById('status'); // Add this ID to a visible status area in HTML
    if (statusBox) {
    statusBox.innerText = "Station Offline";
    statusBox.style.color = "red"; // 🔴 RED for offline
  }
  ...
  // ✅ Optional: reconnect success message
  socket.on('connect', () => {
    console.log("🔌 Reconnected to server.");
    const statusBox = document.getElementById('status');
    if (statusBox) {
    statusBox.innerText = "Station Online";
    statusBox.style.color = "green"; // 🟢 GREEN for online
  }

    reconnecting = false;

    // Resume the previous state if available
    if (lastKnownState) {
      socket.emit('resume_process', lastKnownState);
    }
  });
});
